...
const UploadSubCategoryModel = ({ close, fetchData }) => {
  ...
  const handleRemoveCategorySelected = (categoryId) => {
    setSubCategoryData((preve) => {
      return {
        ...preve,
        category: preve.category.filter((cat) => cat._id !== categoryId), // 🔥 Crée un nouveau tableau SANS la catégorie supprimée
      }
    })
  }
            ...
            <div className='border focus-within:border-primary-200 rounded'>
              ...
              <div className='flex flex-wrap gap-2'>
                {subCategoryData.category.map((cat, index) => {
                  if (!cat) return null // ✅ Ignore les éléments undefined
                  return (
                    <p
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setSubCategoryData((preve) => {
                    const alreadyExists = preve.category.some(
                      (cat) => cat._id === categoryDetails._id
                    )
                    if (alreadyExists) return preve // Si la catégorie existe déjà, on ne fait rien
